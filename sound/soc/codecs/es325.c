...
#define ES325_INTERNAL_ROUTE_MAX 12
...
static u8 es325_internal_route_configs[ES325_INTERNAL_ROUTE_MAX][50] = {
	...
	/* [9]: 2-mic FT ASR(Automated Speech Recognition) */
	{
		0xb0, 0x5c, 0x00, 0x01, /* Algo = Voice processing */
		0xb0, 0x4c, 0x00, 0x01, /* Algo rate = 16 kHz */
		0xb0, 0x5e, 0x00, 0x01, /* Mix rate = 16 kHz */
...
#define ES325_CLK_NAME "xo"
#define ES325_CLK_GET clk_get
#define ES325_CLK_ENALBE clk_prepare_enable
#define ES325_CLK_DISABLE clk_disable
#endif
...
#if 0 // defined(CONFIG_SND_SOC_ES325_I2C)
...
/** ZTE_ADD lvrongguo for test begin */
//       pr_info("%s(): lvrongguo test es325 reset_gpio num : %d\n", __func__,es325->pdata->reset_gpio);
/** ZTE_ADD lvrongguo for test end */
	mdelay(1); 
	...
	if (rc < 0) {
		pr_err("%s(): firmware load failed boot write:rc(%d)\n", __func__, rc);
		return	0; /* rc; */
	}
	msleep(100);
...
static int fw_download(void *arg)
{
	...
	//lvrongguo add for test
       // pr_info("lvrongguo ::: register_snd_soc return value = %d",rc);
	//lvrongguo add for test end
...
static struct snd_kcontrol_new es325_digital_ext_snd_controls[] = {
	...
	SOC_SINGLE_EXT("ES325 Presets",SND_SOC_NOPM, 0, 100, 0,
				...
				enum snd_soc_bias_level level)
{
	...
	codec->dapm.bias_level = level ;
	///BUG_ON(codec->dapm.bias_level ==3);
	pr_info("lvrongguo ::: level = %d",codec->dapm.bias_level);

	/* dev_dbg(codec->dev, "%s(): exit\n", __func__); */
	return ret;
}

#if 0 // defined(CONFIG_SND_SOC_ES325_I2S)
...
EXPORT_SYMBOL_GPL(es325_suspend);
...
EXPORT_SYMBOL_GPL(es325_resume);
#endif
/***lvrongguo delete*/
...
/** ZTE_ADD lvrongguo for es325 sleep test begin */
       pr_info( " es325_codec_suspend****** sleep *****\n");
/** ZTE_ADD lvrongguo for es325 sleep test end */
...
/** ZTE_ADD lvrongguo for es325 wakeup test begin */
       pr_info(" es325_codec_resume****** wakeup *****\n");
/** ZTE_ADD lvrongguo for es325 wakeup test end */
...
static int es325_codec_probe(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "%s(): es325 = 0x%08x\n", __func__,
		 (unsigned int)es325);
	es325->codec = codec;
	//wangweiping add begin
	#if 0
	pr_info("wang es325_codec_probe----\n");
	es325_priv.codec = codec;
	#endif
	//wangweiping add end
...
static int es325_clk_ctl(int enable)
{
	...
	pr_err("chenjun:%s: enable(%d), ret(%d)\n", __func__, enable, ret);
...
static int register_snd_soc(struct es325_priv *priv)
{
	...
	rc = es325_init_slim_slave(sbdev);
       ...
       //lvrongguo add for test
       pr_info("lvrongguo ::: register_snd_soc entry!\n");  
       pr_info("lvrongguo ::: soc_codec_dev_es325 = 0X%x\n" , (int)(&soc_codec_dev_es325));     //success
	//lvrongguo add test end
	dev_dbg(&sbdev->dev, "%s(): name = %s\n", __func__, sbdev->name);
	rc = snd_soc_register_codec(&sbdev->dev, &soc_codec_dev_es325, es325_dai,
				    ARRAY_SIZE(es325_dai));
       //lvrongguo add for test
       pr_info("lvrongguo ::: snd_soc_register_codec return value = %d\n",rc);    //success
	//lvrongguo add test end
...
//wangweiping add test begin
#if 0
static int es325_slim_resume(struct slim_device *sbdev)
{
	struct es325_priv *priv;
	pr_info("wang ----es325_slim_resume\n");
	priv = slim_get_devicedata(sbdev);
	//return es325_codec_resume(es325_priv.codec);
	return es325_wakeup(priv);
}
static int es325_slim_suspend(struct slim_device *sbdev, pm_message_t pmesg)
{
       struct es325_priv *priv;
	//priv = slim_get_devicedata(sbdev);
	pr_info("wang ----es325_slim_suspend\n");
	//return es325_codec_suspend(es325_priv.codec);
	priv = slim_get_devicedata(sbdev);

	return es325_sleep(priv);
}
#endif
//wangweiping add test end
...
static struct slim_driver es325_slim_driver = {
	...
	//wangweiping add for test begin
#if 0
	.resume = es325_slim_resume,
	.suspend = es325_slim_suspend,
#endif
	//wangweiping add for test end
};
#endif
