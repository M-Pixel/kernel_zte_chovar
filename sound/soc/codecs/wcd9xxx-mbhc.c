...
#define WCD9XXX_MEAS_DELTA_MAX_MV 300 // chenjun:orig:50
...
#define WCD9XXX_CS_MEAS_DELTA_MAX_MV 300 // chenjun:orig:10
			  ...
			  unsigned long event_state)
{
	...
	if (event_state & (1 << MBHC_EVENT_PA_HPHL)) {
		...
		pr_err("%s: Invalid, inconsistent HPHL:ch(%d):sz(%d)\n", __func__, ch, sz); // chenjun
		type = PLUG_TYPE_INVALID;
	...
	if (type == PLUG_TYPE_HEADSET && dgnd && !dgnd->mic_bias) {
		if ((dgnd->_vdces + WCD9XXX_CS_GM_SWAP_THRES_MIN_MV <
		    ...
		    (dgnd->_vdces + WCD9XXX_CS_GM_SWAP_THRES_MAX_MV >
		     maxv)) {
			type = PLUG_TYPE_HEADSET; // chenjun:orig:PLUG_TYPE_GND_MIC_SWAP
			pr_err("%s: change GND_MIC_SWAP to type(%d)\n", __func__, type); // chenjun
		}
		else if (dgnd->_type != PLUG_TYPE_HEADSET) {
			pr_err("%s: Invalid, inconsistent types:dgnd use HEADSET\n", __func__); // chenjun
			type = PLUG_TYPE_HEADSET; // chenjun:orig:PLUG_TYPE_INVALID
		}
	}
		       ...
		       unsigned long event_state)
{
	...
	if (event_state & (1 << MBHC_EVENT_PA_HPHL)) {
		...
		pr_err("%s: Invalid, inconsistent HPHL:ch(%d):size(%d)\n", __func__, ch, size); // chenjun
		type = PLUG_TYPE_INVALID;
		recover_plug_type = 1; // chejun
		goto exit;
	}
	...
	if (!(event_state & (1UL << MBHC_EVENT_PA_HPHL))) {
		if (((type == PLUG_TYPE_HEADSET ||
			...
			pr_err("%s: Invalid, not fully inserted, TYPE %d used:ch(%d):size(%d)\n",
				 __func__, type, ch, size); // chenjun
			// chenjun:type = PLUG_TYPE_INVALID;
		}
	}
...
// chenjun:recover plug type
	if (recover_plug_type)
	{
	    ...
	    pr_err("%s: recover done:hph_num(%d):half size(%d):type(%d)\n", __func__, hph_num, (size/2), type); // chenjun

	}
//
	pr_err("%s: Plug type %d detected, micbias_enable %d\n", __func__,
...
static void wcd9xxx_hs_remove_irq_noswch(struct wcd9xxx_mbhc *mbhc)
{
	...
	do {
       ...
       removed = wcd9xxx_swch_level_remove(mbhc); // chenjun
       pr_err("%s: headset %sactually removed\n", __func__,
                 removed ? "" : "not "); // chenjun
	if (cs_enable)
		wcd9xxx_turn_onoff_current_source(mbhc, false, false);
...
static void wcd9xxx_enable_mbhc_txfe(struct wcd9xxx_mbhc *mbhc, bool on)
{
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mbhc_txfe)
		...
		snd_soc_write(mbhc->codec, WCD9XXX_A_TX_7_MBHC_TEST_CTL, 0x78);	
