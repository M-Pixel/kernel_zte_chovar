...
static int sec_mi2s_tx_bit_format = SNDRV_PCM_FORMAT_S16_LE;
...
// chenjun:#define LPAIF_PRI_MODE_MUXSEL (LPAIF_OFFSET + 0x2B000)
#define LPAIF_PRI_MODE_MUXSEL (LPAIF_OFFSET + 0x2C000) // chenjun:PRI
#define LPAIF_SEC_MODE_MUXSEL (LPAIF_OFFSET + 0x2D000) // chenjun:SEC
#define LPAIF_TER_MODE_MUXSEL (LPAIF_OFFSET + 0x2E000) // chenjun:TER
#define LPAIF_QUAD_MODE_MUXSEL (LPAIF_OFFSET + 0x2F000) // chenjun:QUAD
...
struct msm8974_asoc_mach_data {
	...
	/*lgh msm8974_dtparse_mi2s() is not good enough to distinguish pri and secondary port
		it always write data got from dtsi in the following varies, so pri and secondary cannot
		be enable together*/
...
static const struct afe_clk_cfg lpass_default = {
       ...
       Q6AFE_LPASS_OSR_CLK_1_P024_MHZ,
...
#if 0
static char *msm_prim_mi2s_gpio_name[][2] = {
       {"PRIM_MI2S_WS",          "qcom,prim-mi2s-gpio-ws"},
       {"PRIM_MI2S_D0",          "qcom,prim-mi2s-gpio-data0"},
       {"PRIM_MI2S_D1",          "qcom,prim-mi2s-gpio-data1"},
       {"PRIM_MI2S_SCLK",        "qcom,prim-mi2s-gpio-sclk"},
};

static char *msm_prim_mclk_gpio[][2] = {
       {"MI2S_MCLK",             "qcom,prim-mi2s-gpio-mclk"},
};
#endif
static char *msm_sec_mi2s_gpio_name[][2] = {
       {"PRIM_MI2S_WS",          "qcom,sec-mi2s-gpio-ws"},
       {"PRIM_MI2S_D0",          "qcom,sec-mi2s-gpio-data0"},
       {"PRIM_MI2S_D1",          "qcom,sec-mi2s-gpio-data1"},
       {"PRIM_MI2S_SCLK",        "qcom,sec-mi2s-gpio-sclk"},
};

static char *msm_sec_mclk_gpio[][2] = {
...
/* ZTE_Audio_CJ_130924, chenjun, 2013-09-24, start */
/* use ext TPA2015 speaker amp */
static int ext_spk_amp_en1_gpio = -1;
static int ext_spk_amp_en2_gpio = -1;
/* ZTE_Audio_CJ_130924, chenjun, 2013-09-24, end */
...
/* lgh not distinguish pri and sec now */
static int msm8974_mi2s_tx_ch = 1;
...
/* ZTE_Audio_CJ_130924, chenjun, 2013-09-24, start */
/* use ext TPA2015 speaker amp */
static int msm8974_ext_spk_amp_gpio_init(void)
{
	int ret = 0;

	ext_spk_amp_en1_gpio = of_get_named_gpio(spdev->dev.of_node,
		"qcom,ext-spk-amp-en1-gpio", 0);

	ext_spk_amp_en2_gpio = of_get_named_gpio(spdev->dev.of_node,
		"qcom,ext-spk-amp-en2-gpio", 0);

pr_err("%s: ext-spk-amp-gpio: en1(%d), en2(%d)\n", __func__, ext_spk_amp_en1_gpio, ext_spk_amp_en2_gpio);

	if (ext_spk_amp_en1_gpio >= 0) {
		ret = gpio_request(ext_spk_amp_en1_gpio, "ext_spk_amp_en1_gpio");
		if (ret) {
			pr_err("%s: gpio_request failed for ext_spk_amp_en1_gpio.\n",
				__func__);
			return 0;
		}
		gpio_direction_output(ext_spk_amp_en1_gpio, 1);

		// gpio_set_value_cansleep(ext_spk_amp_en1_gpio, 0);
	
		// pr_err("%s: spk amp en1 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en1_gpio));	

	}

	if (ext_spk_amp_en2_gpio >= 0) {
		ret = gpio_request(ext_spk_amp_en2_gpio, "ext_spk_amp_en2_gpio");
		if (ret) {
			pr_err("%s: gpio_request failed for ext_spk_amp_en2_gpio.\n",
				__func__);
			return 0;
		}
		gpio_direction_output(ext_spk_amp_en2_gpio, 1);

		// gpio_set_value_cansleep(ext_spk_amp_en2_gpio, 0);
	
		// pr_err("%s: spk amp en2 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en2_gpio));	
...
static void msm8974_ext_spk_amp_gpio_enable(u32 on)
{
	if (on) {
		gpio_direction_output(ext_spk_amp_en1_gpio, 0);

		// gpio_set_value_cansleep(ext_spk_amp_en1_gpio, 1);
	
		// pr_err("%s:on:spk amp en1 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en1_gpio));	

		gpio_direction_output(ext_spk_amp_en2_gpio, 0);

		// gpio_set_value_cansleep(ext_spk_amp_en2_gpio, 1);
	
		// pr_err("%s:on:spk amp en2 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en2_gpio));	
		...
		// pr_err("%s:off:spk amp en1 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en1_gpio));	

		gpio_direction_output(ext_spk_amp_en2_gpio, 1);

		// gpio_set_value_cansleep(ext_spk_amp_en2_gpio, 0);
	
		// pr_err("%s:off:spk amp en2 gpio level = %d\n", __func__, __gpio_get_value(ext_spk_amp_en2_gpio));	
	...
	pr_err("%s: %s external speaker PAs' GPIOs.\n", __func__,
			on ? "Enable" : "Disable");
}

static void msm8974_ext_spk_amp_gpio_on(u32 spk)
{

	pr_err("%s:on:spk(0x%x)\n", __func__, spk);

	if (!gpio_is_valid(ext_spk_amp_en1_gpio) || !gpio_is_valid(ext_spk_amp_en2_gpio)) {
		pr_err("%s: ext spk amp en gpio isn't configured\n", __func__);
		...
		{
			pr_err("%s: Turn on ext spk amp. spk = 0x%x\n",
				 __func__, spk);
			msm8974_ext_spk_amp_gpio_enable(1);
		}
	}
}

static void msm8974_ext_spk_amp_gpio_off(u32 spk)
{

	pr_err("%s:off:spk(0x%x)\n", __func__, spk);

	if (!gpio_is_valid(ext_spk_amp_en1_gpio) || !gpio_is_valid(ext_spk_amp_en2_gpio)) {
		pr_err("%s: ext spk amp en gpio isn't configured\n", __func__);
	} else if (spk & (LO_1_SPK_AMP | LO_3_SPK_AMP)) {
		if (msm8974_ext_spk_pamp) {
			pr_err("%s: Turn off ext spk amp. spk = 0x%x\n",
				 __func__, spk);
			msm8974_ext_spk_amp_gpio_enable(0);
...
/* use ext TPA2015 speaker amp */
  ...
  if (gpio_is_valid(ext_spk_amp_en1_gpio) && gpio_is_valid(ext_spk_amp_en2_gpio))
  {
      pr_err("%s:chenjun:on:use ext spk amp en gpio\n", __func__);
      msm8974_ext_spk_amp_gpio_on(spk);
  }
  else
  {
...
/* use ext TPA2015 speaker amp */
  ...
  if (gpio_is_valid(ext_spk_amp_en1_gpio) && gpio_is_valid(ext_spk_amp_en2_gpio))
  {
      pr_err("%s:chenjun:off:use ext spk amp en gpio\n", __func__);
      msm8974_ext_spk_amp_gpio_off(spk);
  }
  else
  {
	if (gpio_is_valid(ext_spk_amp_gpio))
...
static int sec_mi2s_tx_bit_format_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	switch (sec_mi2s_tx_bit_format) {
	case SNDRV_PCM_FORMAT_S24_LE:
		ucontrol->value.integer.value[0] = 1;
		break;
	case SNDRV_PCM_FORMAT_S16_LE:
	...
	pr_debug("%s: sec_mi2s_tx_bit_format = %d, ucontrol value = %ld\n",
			 __func__, sec_mi2s_tx_bit_format,
			ucontrol->value.integer.value[0]);
	return 0;
}

static int sec_mi2s_tx_bit_format_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	switch (ucontrol->value.integer.value[0]) {
	case 1:
		sec_mi2s_tx_bit_format = SNDRV_PCM_FORMAT_S24_LE;
		break;
	case 0:
	default:
		sec_mi2s_tx_bit_format = SNDRV_PCM_FORMAT_S16_LE;
		break;
	}
	pr_debug("%s: sec_mi2s_tx_bit_format = %d, ucontrol value = %ld\n",
				 __func__, sec_mi2s_tx_bit_format,
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	case 8000:
		...
		msm8974_auxpcm_rate = 8000; // chenjun
		break;
	case 16000:
		msm_btsco_rate = BTSCO_RATE_16KHZ;
		msm8974_auxpcm_rate = 16000; // chenjun
		break;
	default:
		msm_btsco_rate = BTSCO_RATE_8KHZ;
		msm8974_auxpcm_rate = 8000; // chenjun
...
pr_err("%s:add rx dev\n", __func__);
	...
	pr_debug("%s:enable xxxxx= %x\n", __func__, enable);
	if (enable) {
		if (pdata->prim_clk_usrs == 0) {
			lpass_clk->clk_val2 = Q6AFE_LPASS_OSR_CLK_1_P024_MHZ;
			lpass_clk->clk_val1 = Q6AFE_LPASS_IBIT_CLK_256_KHZ;
		...
		ret =
		   afe_set_lpass_clock(AFE_PORT_ID_SECONDARY_MI2S_TX, lpass_clk);
		if (ret < 0)
...
/*lgh hard code replace lpaif_pri_muxsel_virt_addr with lpaif_sec_muxsel_virt_addr*/
static int msm_mi2s_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	struct snd_soc_dai *codec_dai = rtd->codec_dai; // chenjun
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	...
	if (atomic_inc_return(&mi2s_ref_count) == 1) {
		pcm_sel_reg = ioread32(lpaif_sec_muxsel_virt_addr);
...
/*lgh hard code replace pri_mi2s_tx_bit_format fucking with lpaif_sec_muxsel_virt_addr */
static int msm8974_mi2s_tx_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
					     struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
						SNDRV_PCM_HW_PARAM_RATE);
	struct snd_interval *channels = hw_param_interval(params,
						SNDRV_PCM_HW_PARAM_CHANNELS);
	param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
				sec_mi2s_tx_bit_format);
	/*lgh hard code to 8K here*/
//	rate->min = rate->max = 48000;
	rate->min = rate->max = 8000;
	channels->min = channels->max = msm8974_mi2s_tx_ch;
...
static int msm8974_mi2s_rx_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
					     struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
						SNDRV_PCM_HW_PARAM_RATE);
...
static int msm8974_mi2s_tx_ch_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm8974_i2s_tx_ch  = %d\n", __func__,
		 msm8974_mi2s_tx_ch);
	ucontrol->value.integer.value[0] = msm8974_mi2s_tx_ch - 1;
	return 0;
}

static int msm8974_mi2s_tx_ch_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	msm8974_mi2s_tx_ch = ucontrol->value.integer.value[0] + 1;
	pr_debug("%s: msm8974_i2s_tx_ch = %d\n", __func__,
		 msm8974_mi2s_tx_ch);
	return 1;
}
...
static const struct soc_enum msm8974_mi2s_enum[] = {
...
// chenjun
	SOC_ENUM_EXT("SLIM_3_RX Channels", msm_snd_enum[1],
			msm_slim_0_rx_ch_get, msm_slim_0_rx_ch_put),
//
	SOC_ENUM_EXT("PRI_MI2S_TX Channels",   msm8974_mi2s_enum[0],
			msm8974_mi2s_tx_ch_get,
			msm8974_mi2s_tx_ch_put),
	SOC_ENUM_EXT("PRI_MI2S_TX Format", msm8974_mi2s_enum[1],
			pri_mi2s_tx_bit_format_get,
			pri_mi2s_tx_bit_format_put),
	SOC_ENUM_EXT("SEC_MI2S_TX Channels",   msm8974_mi2s_enum[0],
			msm8974_mi2s_tx_ch_get,
			msm8974_mi2s_tx_ch_put),
	SOC_ENUM_EXT("SEC_MI2S_TX Format", msm8974_mi2s_enum[1],
			sec_mi2s_tx_bit_format_get,
			sec_mi2s_tx_bit_format_put),
};
...
//
	/* MI2S Backend DAI Links */
	{
		...
		.be_hw_params_fixup = &msm8974_mi2s_tx_be_hw_params_fixup,
...
/* MI2S Backend DAI Links */
	...
	},
	{
		...
		.be_hw_params_fixup = msm8974_mi2s_rx_be_hw_params_fixup,
		.ops = &msm8974_mi2s_be_ops,
	},
};
				...
				char *msm_mi2s_gpio_name[][2])
{
	...
	/*lgh now pri and sec I2S port cannot co-exist*/
	for (i = 0; i < ARRAY_SIZE(msm_sec_mclk_gpio); i++) {
		dt_mclk = of_get_named_gpio_flags(pdev->dev.of_node,
				msm_sec_mclk_gpio[i][DT_PARSE_INDEX], 0,
				&flags);
		if (dt_mclk > 0) {
			mclk_pin->gpio_name =
				msm_sec_mclk_gpio[i][GPIO_NAME_INDEX];
...
static __devinit int msm8974_asoc_machine_probe(struct platform_device *pdev)
{
	...
	/*lgh prim and sec cannot co-exist now*/
//	ret = msm8974_dtparse_mi2s(pdev, &pdata, &pdata->mi2s_ctrl,
//					msm_prim_mi2s_gpio_name);
		ret = msm8974_dtparse_mi2s(pdev, &pdata, &pdata->mi2s_ctrl,
					msm_sec_mi2s_gpio_name);
	if (ret) {
		dev_err(&pdev->dev,
