...
#include "synaptics_dsx_i2c_s3400.h"
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi = &(rmi4_data->rmi4_mod_info);
	//printk("synaptics_rmi4_sensor_report f01_data_base_addr:0x%x,num_of_intr_regs:0x%x\n",
		//rmi4_data->f01_data_base_addr,rmi4_data->num_of_intr_regs);
	msleep(10);
	/*
	...
	if (retval < 0) {
		...
		//for (i=0;i<rmi4_data->num_of_intr_regs + 1;i++)
		//	pr_info("addr=0x%x, buf[%d]=0x%x\n",rmi4_data->f01_data_base_addr+i,i,data[i]);
 ...
 * is detected.
 */
static irqreturn_t synaptics_rmi4_irq_s3400(int irq, void *data)
{
		...
		bool enable)
{
	...
	if (enable) {
		if (rmi4_data->irq_enabled)
			return retval;
	printk("synaptics_rmi4_irq_enable :%d\n",enable);
		...
		if (retval < 0)
			return retval;
		printk("synaptics_rmi4_irq_enable num_of_intr_regs:%d\n",
			rmi4_data->num_of_intr_regs);
		printk("synaptics_rmi4_irq_enable irq_flags:0x%lx\n",platform_data->irq_flags);
		
		retval = request_threaded_irq(rmi4_data->irq, NULL,
				synaptics_rmi4_irq_s3400, platform_data->irq_flags,
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			dev_err(&rmi4_data->i2c_client->dev,
					...
					page_number);
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: querybaseaddr:%02x,ctrlbaseaddr:%02x,databaseaddr:%02x,intr_count:%02x\n",
					__func__, rmi_fd.query_base_addr,rmi_fd.ctrl_base_addr,
...
static void touchscreen_en( int hl )
{
	gpio_direction_output(GPIO_TS_EN, hl);
	return;
}
...
#endif
#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_FW_UPDATE_S3400)
...
extern int rmi4_fw_update_module_init_s3400(void);
...
extern int rmidev_module_init_s3400(void);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_FW_UPDATE_S3400
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (!i2c_check_functionality(client->adapter,
		...
		syna_parse_dt(&client->dev, platform_data);
#else	
...
//huangjinyu ok
	...
	#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_FW_UPDATE_S3400)
	rmi4_fw_update_module_init_s3400();
	syna_fwupdate_init(client);
	#endif	
	#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_RMI4_DEV)
	rmidev_module_init_s3400();
	#endif
