...
/*ergate*/
static int last_down_fingers = 0, down_fingers = 0;
//extern void dbs_touchscreen_callback(int state);
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		if (will_report) {
			...
			for_each_set_bit(i, pointers_to_report, NR_FINGERS) {
				...
				//printk("touch (%d,%d)\n",new_report_data[i].x,new_report_data[i].y);
				/*ergate*/
				down_fingers++;
				}
			...
			/*ergate*/
			if(last_down_fingers == 0 && down_fingers > 0)
			{
			printk("touch down\n");
			//dbs_touchscreen_callback(1);
			}
			else if(last_down_fingers > 0 && down_fingers == 0)
			{
			printk("touch up\n");
			//dbs_touchscreen_callback(0);
			}
			last_down_fingers = down_fingers;
...
/*ergate*/
#ifdef CONFIG_OF
static int syna_parse_dt(struct device *dev, struct synaptics_rmi_data *pdata)
{
#if 0
	int rc;
...
//	uint max_x,max_y;
	...
	client->irq = gpio_to_irq(GPIO_TS_IRQ);
	...
	pdata = devm_kzalloc(&client->dev,
		sizeof(struct synaptics_rmi_data), GFP_KERNEL);
	if (!pdata) {
		dev_err(&client->dev, "Failed to allocate memory\n");
		goto err_power_failed; 
	}
	syna_parse_dt(&client->dev, pdata);
#else	
...
static int synaptics_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(ts == NULL)
	{
		printk("%s:touchscreen not found\n",__func__);
		return 0;
	}
