...
static int syna_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	//prop					= of_find_property(dev->of_node,"synaptics,max_y",NULL);
	//if(prop&&(prop->value))
	//memcpy(&(pdata->maxy_offset), prop->value, 1);
			  ...
			  void *data)
{
	...
	len +=sprintf(page + len, "%d%s%d%s%d%s%d%s%d%s%d%s%s%d\n",
	touchscreen_logs[i].line,touchscreen_logs[i].space,
	touchscreen_logs[i].month,touchscreen_logs[i].date,
	touchscreen_logs[i].day,touchscreen_logs[i].space1,touchscreen_logs[i].hour,
	touchscreen_logs[i].time1,touchscreen_logs[i].min,touchscreen_logs[i].time2,
	touchscreen_logs[i].second,touchscreen_logs[i].space2,touchscreen_logs[i].fingers,
