...
static int synaptics_check_config_id(struct synaptics_rmi4_data *rmi4_data, const unsigned char *pfwfile )
{
	...
	firmware_imgsize = extract_uint_le(&(pfwfile[8]));
...
//		}
		...
		snprintf(fwu->image_name, NAME_BUFFER_SIZE, "%s", syna_file_name);
		dev_info(&fwu->rmi4_data->i2c_client->dev,
...
int syna_get_fw_ver(struct i2c_client *client, char *pfwfilename)
{
	...
	firmware_imgsize = extract_uint_le(fw_image+8);
...
int  rmi4_fw_update_module_init_s3400(void)
{
