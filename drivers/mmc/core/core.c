...
void mmc_rescan(struct work_struct *work)
{
	...
	if(host->index == 2) {
		if(check_detect_flag()) {
			printk("shaohua enable detect, continue\n");
			clear_detect_flag();
		} else {
			printk("shaohua didn't enable detect, skip\n");
			return;
		}
	}
#endif
...
//ZTE_yeganlin_20130911,merged the function of polling
...
#ifdef CONFIG_BCMDHD
		if((host->index == 2) && wifi_clk) {
			printk("sh return clk, %d \n", wifi_clk);
			host->ios.clock = wifi_clk;
			wifi_clk = 0;
			mmc_set_ios(host);
		}
#endif
