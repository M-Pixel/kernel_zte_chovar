...
bool coresight_fuse_access_disabled(void)
{
	...
	dev_info(drvdata->dev, "config0: %lx\n", (unsigned long)config0);
	dev_info(drvdata->dev, "config1: %lx\n", (unsigned long)config1);
	...
	if (ret)
		dev_info(drvdata->dev, "coresight fuse disabled\n");
...
bool coresight_fuse_apps_access_disabled(void)
{
	...
	dev_info(drvdata->dev, "apps config0: %lx\n", (unsigned long)config0);
	dev_info(drvdata->dev, "apps config1: %lx\n", (unsigned long)config1);
	...
	if (ret)
		dev_info(drvdata->dev, "apps fuse disabled\n");
