...
//tcd 
static bool freqing = false;
int cpufreq_disabled(void)
{
	return off;
}
void disable_cpufreq(bool b)
{
    if (b == true) {
	/* we will wait until freq change is finished */
	if (freqing == true)
		msleep_interruptible(10);
	/* any freqing change will finish in 5ms, so max 10ms waited*/
	off = 1;
    } else
	off = 0;
}
