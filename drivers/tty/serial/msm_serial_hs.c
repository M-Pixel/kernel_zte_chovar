...
#if 0 //bt_dm debug for brcm4339 lpm
#define DMBT_DBG  
#endif
#ifdef DMBT_DBG
#define dm_err printk
#else
#define dm_err(fmt, arg...)
#endif
...
printk("%s msm_uport %s NULL\n",__func__,msm_uport?"!=":"==");
printk("%s clk_count:%d rc %d\n",__func__,atomic_read(&msm_uport->clk_count),rc);
	if (rc < 0) {
		...
		/* to balance clk_state  zte origin not have, sansung & LG  have  */
				 ...
				 unsigned int value)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	unsigned int offset;
dm_err("%s index:0x%x   value:0x%x\n",__func__,index,value);
	offset = *(msm_uport->reg_ptr + index);
dm_err("%s offset:0x%x\n",__func__,offset);
...
//        int i;
//	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
dm_err("%s: ^_^ uport %s NULL\n",__func__,uport?"!=":"==");
#if 0 //bt_dm for debug 4339 lmp
for (i=0; i<UART_DM_LAST; i++)
{
	dm_err(" 0x%x\n",*(msm_uport->reg_ptr+i));
	if (i+1 == UART_DM_LAST)
	dm_err("\n");
	...
	msm_hs_clock_unvote(msm_uport);
#else
   data = msm_hs_read(uport, UART_DM_SR/*UARTDM_SR_ADDR*/);
#endif
dm_err("%s data:0x%x\n",__func__,data);
...
/* lahg add calling callback for bluesleep here */
//void bluesleep_outgoing_data(void);
#ifdef CONFIG_BT_BCM_BTLA_SLEEP//CONFIG_BT_BCM_SLEEP//
	...
	*msm_hs_dt_to_pdata(struct platform_device *pdev)
{
	...
	MSM_HS_DBG("tx_ep_pipe_index:%d rx_ep_pipe_index:%d\n"
		...
		pdata->uart_rfr_gpio);
	dm_err("%s: tx_ep_pipe_index:%d rx_ep_pipe_index:%d\n"
		"tx_gpio:%d rx_gpio:%d rfr_gpio:%d cts_gpio:%d pdata->inject_rx_on_wakeup %d\n"
		"pdata->rx_to_inject %c",__func__,
		pdata->bam_tx_ep_pipe_index, pdata->bam_rx_ep_pipe_index,
		pdata->uart_tx_gpio, pdata->uart_rx_gpio, pdata->uart_cts_gpio,
		pdata->uart_rfr_gpio,pdata->inject_rx_on_wakeup,pdata->rx_to_inject);
...
static int __devinit msm_hs_probe(struct platform_device *pdev)
{
	...
	dm_err("%s ^_^pdata->wakeup_irq:%d\n",__func__,pdata->wakeup_irq);
