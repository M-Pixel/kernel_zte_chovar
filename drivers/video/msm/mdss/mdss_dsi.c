	...
	struct dss_module_power *mp)
{
	...
	for_each_child_of_node(of_node, supply_node) {
		if (!strncmp(supply_node->name, "qcom,platform-supply-entry",
						26)){
				if(!of_property_read_bool(supply_node,
					"zte,supply_disabled")){
					++mp->num_vreg;
				}
		}
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->lcd_5v_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
		"zte,lcd-5v-enable-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_5v_en_gpio)) {
		pr_err("%s:%d, qcom,lcd-5v-enable-gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->lcd_5v_en_gpio, "lcd_5v_en_gpio");
		if (rc) {
			pr_err("request lcd_5v_en_gpio failed, rc=%d\n",
			       rc);
