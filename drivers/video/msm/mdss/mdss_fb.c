...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	mfd->bl_level = 0;
	mfd->bl_scale = 1024;
	mfd->bl_min_lvl = mfd->panel_info->bl_min;//30 yichangming modify
...
#if 0
static struct workqueue_struct *lcd_bkl_workq;
struct mfd_dw {
        struct msm_fb_data_type *mfd;
        struct delayed_work timeout_work;
}mfd_bkl;
static void mdss_fb_bkl_workqueue_cb(struct work_struct *work)
{
	struct delayed_work *dw = to_delayed_work(work);
	struct mfd_dw *mfd_dw1 = container_of(dw,
		struct mfd_dw, timeout_work);
	struct msm_fb_data_type *mfd = mfd_dw1->mfd;
	if (!mfd) {
		pr_err("%s mfd NULL\n", __func__);
		return;
	}
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if (((!mfd->panel_power_on && mfd->dcm_state != DCM_ENTER)
		...
		if(!lcd_bkl_workq)
		{
			lcd_bkl_workq = create_workqueue("lcd_bkl_workq");
			INIT_DELAYED_WORK(&mfd_bkl.timeout_work, mdss_fb_bkl_workqueue_cb);
		}
		mfd_bkl.mfd=mfd;
		queue_delayed_work(lcd_bkl_workq,&mfd_bkl.timeout_work, msecs_to_jiffies(50));
	#endif
		return;
	} else {
