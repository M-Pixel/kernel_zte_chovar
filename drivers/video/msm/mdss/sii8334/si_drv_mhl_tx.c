...
extern struct mhl_tx_ctrl *mhl_ctrl ;
//
...
void SiiMhlTxDrvAcquireUpstreamHPDControlDriveLow (void)
{
	...
	if (mhl_ctrl && mhl_ctrl->hdmi_mhl_ops) {
			rc = mhl_ctrl->hdmi_mhl_ops->set_upstream_hpd(mhl_ctrl->hdmi_pdev, 0);
...
static int IntToString(int n, char * string, int radix)
{
 	int i=0;
	int j;
...
static int mhl_chip_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0,total = 0;
	char mhl_chip_info[20] = {0};
	char *mhl_tag = "mhlversion:";
	memcpy(mhl_chip_info,mhl_tag,strlen(mhl_tag));
	total = strlen(mhl_tag);
	len = IntToString((g_chipDeviceId>>8) & 0xff,mhl_chip_info+total,16);  
	mhl_chip_info[total + len]= ' ';
	total += len+1;
	len = IntToString(g_chipDeviceId & 0xff,mhl_chip_info+total,16);  
	mhl_chip_info[total + len]= ' ';
	total += len+1;
	IntToString(g_chipRevId,mhl_chip_info+total,16);  
	len = sprintf(page, "%s\n", mhl_chip_info);
...
static void  mhl_chip_proc(void)
{	
	static struct proc_dir_entry * d_entry;
	d_entry = create_proc_entry("mhl_version", 0, NULL);
	if(d_entry) {	
		d_entry->read_proc = mhl_chip_read_proc;
...
//	    TX_DEBUG_PRINT(("Drv: int4Status: 0x%02X\n", (int) SiiRegRead(REG_INTR4)));
   ...
   g_chipDeviceId = (SiiRegRead(TX_PAGE_L0 | 0x03) << 8) |SiiRegRead(TX_PAGE_L0 | 0x02);
	mhl_chip_proc();
...
void ProcessRgnd (void)
{
	...
	if (0x02 == rgndImpedance)
	{
		...
		if (mhl_ctrl->hdmi_mhl_ops) {
			rc =mhl_ctrl-> hdmi_mhl_ops->set_upstream_hpd(
				mhl_ctrl->hdmi_pdev, 1);
