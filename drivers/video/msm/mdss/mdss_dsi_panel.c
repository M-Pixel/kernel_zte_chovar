...
void  mdss_dsi_panel_lcd_proc(const char * panel_name)
{	
	...
	if (!panel_name){
		pr_info("%s:%d, lcd proc dir panel name not specified\n",
						__func__, __LINE__);
		strcpy(module_name,"0");
	}else{
		pr_info("%s: lcd proc Panel Name = %s\n", __func__, panel_name);
...
static void mdss_dsi_panel_bklt_gpio(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	level &= 0x1f;
   	...
   	if(level==0){
    		gpio_direction_output(ctrl->bl_cmd_gpio, 0);
		mdelay(3);
		pr_err("set backlight to level %d\n",level);
		onewiremode = false;
			
    	}else {
		if(!onewiremode)	
		{
			pr_err("before select_1wire_mode\n");
...
void mdss_dsi_panel_5v_power(struct mdss_panel_data *pdata, int enable)
{
	...
	if (!gpio_is_valid(ctrl_pdata->lcd_5v_en_gpio)) {
		pr_debug("%s:%d, lcd_5v_en_gpio not configured\n",
			   __func__, __LINE__);
		return ;
	}
	...
	if (enable) {
		if (gpio_is_valid(ctrl_pdata->lcd_5v_en_gpio)){
			gpio_set_value((ctrl_pdata->lcd_5v_en_gpio), 1);
			msleep(5);
		}
		
	} else {
		if (gpio_is_valid(ctrl_pdata->lcd_5v_en_gpio))
			gpio_set_value((ctrl_pdata->lcd_5v_en_gpio), 0);
	}
}
...
/*
static int mdss_panel_parse_dt_compare_id(struct platform_device *pdev,u32 *id)
{
	int rc;
	u32 lcd_product = 0;
	bool default_lcd_enabled = false;
	if(*id){
		rc = of_property_read_u32(pdev->dev.of_node, "zte,lcd-product", &lcd_product);
		if (rc) {
		pr_err("%s:%d, panel iterm id not specified\n",
							__func__, __LINE__);
		return -EINVAL;
		}
		printk("[lcd_info]current detected panel dts is lcd_product = %d \n",lcd_product);
	
		if(lcd_product == *id)
			return 0;
	}else{
			default_lcd_enabled = of_property_read_bool(pdev->dev.of_node,
					"zte,default_lcd_enabled");
			if (!default_lcd_enabled) {
				return  -EINVAL;
			} else {
				rc = of_property_read_u32(pdev->dev.of_node, "zte,lcd-product", &lcd_product);
				if (rc) {
					pr_err("%s:%d, qcom,lcd-product not specified\n",
							__func__, __LINE__);
				}else{
					*id = lcd_product;
				}
...
/*
	rc = mdss_panel_parse_dt_compare_id(pdev,&LcdPanleID);
...
*/
	...
	mdss_dsi_panel_lcd_proc(panel_name);
	return 0;
}
