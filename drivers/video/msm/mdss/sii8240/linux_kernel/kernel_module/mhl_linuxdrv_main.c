...
{
	int rc;
	static struct regulator*reg_8941_l12;	
	static struct regulator*reg_8941_s3;
	static int sil8240_ldo_3P3_gpio = 0;


	if(!sil8240_ldo_3P3_gpio ){
		
		sil8240_ldo_3P3_gpio = of_get_named_gpio(dev->of_node, "mhl-ldo-gpio", 0);
		
		if (sil8240_ldo_3P3_gpio < 0) {
			pr_err("%s: Can't get mhl-ldo-gpio\n", __func__);
			return-ENODEV;
		}

		rc = gpio_request(sil8240_ldo_3P3_gpio, "sil8240_ldo_3P3_gpio");
		 
		if (rc) {
			pr_err("%s:Failed to request GPIO %d\n",
					__func__, sil8240_ldo_3P3_gpio);
			return-ENODEV;		
		}
	}
	...
	if (!reg_8941_l12) {
		...
		reg_8941_l12 = regulator_get(dev, "avcc_12");
	...
	if (!reg_8941_s3) {
			...
			reg_8941_s3 = regulator_get(dev, "avcc_18");
			if (IS_ERR(reg_8941_s3)) 
				pr_err("could not get reg_8941_s4, rc = %ld\n",PTR_ERR(reg_8941_s3));
			
			rc = regulator_set_voltage(reg_8941_s3, 1800000, 1800000);
			if (rc) 
				pr_err("set_voltage failed for 8941_s3, rc=%d\n", rc);
    	...
    	if (on)
	{
		...
		rc = regulator_enable(reg_8941_s3);
		if (rc) 
			pr_err("%s: regulator_enable of reg_8941_s3 failed(%d)\n",__func__, rc);
		
		gpio_direction_output(sil8240_ldo_3P3_gpio,1);
		
      }
	else
	{

		rc = regulator_disable(reg_8941_l12);
		if (rc)
			pr_err("disable reg_8941_l12 failed, rc=%d\n", rc);

		rc = regulator_disable(reg_8941_s3);
		if (rc)
			pr_err("disable reg_8941_s3 failed, rc=%d\n", rc);

		gpio_direction_output(sil8240_ldo_3P3_gpio,0);
...
{/*
   ...
   gDriverContext.pRcpDevice = rmt_input;
    ...
    if (input_register_device(rmt_input)) {
	...
	switch(eventCode) {
		...
		case MHL_TX_EVENT_POW_BIT_CHG:	// Peer's power capability has changed
			if (eventParam)
			{
				...
				printk("MHLEVENT=MHL VBUS power on, you should close your OTG power output here");
			}
			else
			{
				printk("MHLEVENT=MHL VBUS power off, you should keep your OTG power output here");
