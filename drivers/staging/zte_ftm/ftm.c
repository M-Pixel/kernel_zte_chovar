...
#if 0	
	__raw_writel(FUSION_MAGIC_NUM_1, MSM_IMEM_BASE+0x24);
	__raw_writel(FUSION_MAGIC_NUM_2, MSM_IMEM_BASE+0x28);
	__raw_writel(fusion_type, MSM_IMEM_BASE+0x2c);
	__raw_writel(QFPROM_STATUS_UNKOWN, MSM_IMEM_BASE+0x30);
	mb();		
#else
...
static ssize_t show_dump(struct sys_device *dev, struct sysdev_attribute *attr, char *buf)
{
	...
	max_size = PAGE_SIZE;
	used_size = 0;
	for (cmd_buf.row_address = 0xFC4B80A0; cmd_buf.row_address <0xFC4B8440 ; cmd_buf.row_address += 8) 
	{
