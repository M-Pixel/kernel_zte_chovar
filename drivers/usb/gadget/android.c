	...
	*/
	while (b) {
		...
		if (name) {
			...
			err = gserial_init_port(ports, name, name);//change xport_name to name
			if (err) {
					...
					struct usb_composite_dev *cdev)
{
	...
	/*"lun":sdcard  "lun_in":phone  "lun_cd":cdrom */
	config->fsg.nluns = 1;
	name[0] = "lun";
	...
	/*for compatibility of ums implementation with 
	internal and external sdcard(s)*/
	config->fsg.nluns = 2;
