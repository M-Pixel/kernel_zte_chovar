...
static int dwc3_otg_set_power(struct usb_phy *phy, unsigned mA)
{
	...
	if (dotg->charger->max_power <= 2 && mA > 2) {
		...
		if (power_supply_set_online(dotg->psy, true)){
			printk(KERN_INFO "Enable charging to call power_supply_set_online failed %s\n",__func__);
			goto psy_error;
		}
		
		if (power_supply_set_current_limit(dotg->psy, 1000*mA)){
			printk(KERN_INFO "Enable charging to call power_supply_set_current_limit failed %s\n",__func__);
			goto psy_error;
		}
	} else if (dotg->charger->max_power > 0 && (mA == 0 || mA == 2)) {
		/* Disable charging */
		if (power_supply_set_online(dotg->psy, false)){
			printk(KERN_INFO "Disable charging  to call power_supply_set_online failed %s\n",__func__);
			goto psy_error;
		}
		/* Set max current limit */
		if (power_supply_set_current_limit(dotg->psy, 0)){
			printk(KERN_INFO "Set max current limit to call power_supply_set_current_limit failed %s\n",__func__);
			goto psy_error;
		}
	}
