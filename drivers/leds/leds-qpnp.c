...
/* Flags for Look Up Table in pwm.h zte */
//#define PM_PWM_LUT_LOOP			0x01	loop for ever
//#define PM_PWM_LUT_RAMP_UP		0x02	first goes up ½¥ÁÁ! else ½¥Ãð!
//#define PM_PWM_LUT_REVERSE			0x04	gose up then slow down. else gose up then off
...
static int
qpnp_led_masked_read(struct qpnp_led_data *led, u16 addr, u8 mask, u8 *val)
{
	int rc;
	u8 reg;
...
static int read_wled_default_brightness(struct qpnp_led_data *led)
{
	u8 val = 0;
	int rc = 0;
	rc = qpnp_led_masked_read(led, WLED_BRIGHTNESS_CNTL_MSB(led->base, 1), WLED_MSB_MASK,&val);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
	...
	rc = qpnp_led_masked_read(led, WLED_BRIGHTNESS_CNTL_LSB(led->base, 1), 0xff,&val);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
				"WLED set brightness LSB failed(%d)\n", rc);
		return rc;
	}
	led->cdev.brightness <<= 8;
	led->cdev.brightness |= val;
	return 0;
...
static int qpnp_rgb_set(struct qpnp_led_data *led)
{
	...
	printk(LED_DRIVER_TAG "qpnp_rgb_set() brightness=%d; \t %s\n",led->cdev.brightness,led->cdev.name);	//zte-ccb-20130903

	if (led->cdev.brightness) {
		if (!led->rgb_cfg->pwm_cfg->blinking)	//in led_blink(ture) set it ture / false ! zte
...
static int __devinit qpnp_led_set_max_brightness(struct qpnp_led_data *led)
{
	...
	case QPNP_ID_LED_MPP:
		...
		break;
	case QPNP_ID_KPDBL:
		led->cdev.max_brightness = 60;//KPDBL_MAX_LEVEL;
		break;
	default:
		dev_err(&led->spmi_dev->dev, "Invalid LED(%d)\n", led->id);
		return -EINVAL;
	...
	const char *buf, size_t count)
{
	...
	printk(LED_DRIVER_TAG "pause_lo_store() set to %d\n",pause_lo);	//zte-ccb-20130903
	...
	const char *buf, size_t count)
{
	...
	printk(LED_DRIVER_TAG "pause_hi_store() set to%d\n",pause_hi);	//zte-ccb-20130903
...
static ssize_t idx_len_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct qpnp_led_data *led;
	...
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	ssize_t ret;
	u32 previous_idx_len;
	struct pwm_config_data *pwm_cfg;

	ret = kstrtou32(buf, 10, &idx_len);
	if (ret)
		return ret;
	led = container_of(led_cdev, struct qpnp_led_data, cdev);
	...
	printk(LED_DRIVER_TAG "idx_len_store() idx_len=%d\n",idx_len);	//zte-ccb-20130903
	
	if (pwm_cfg->mode == LPG_MODE)
		pwm_cfg->blinking = true;

	previous_idx_len = pwm_cfg->lut_params.idx_len;
	pwm_cfg->lut_params.idx_len = idx_len;
	pwm_free(pwm_cfg->pwm_dev);
	ret = qpnp_pwm_init(pwm_cfg, led->spmi_dev, led->cdev.name);
	if (ret) {
			...
			struct pwm_config_data *pwm_cfg)
{
	if (pwm_cfg->use_blink) {		
		if (led->cdev.brightness) {
			...
			pwm_cfg->mode = pwm_cfg->default_mode;	//retore the default mode also in qpnp_rgb_set(brightness=0) zte
			if (led->id == QPNP_ID_LED_MPP)
				led->mpp_cfg->pwm_mode = pwm_cfg->default_mode;
		}

		led->cdev.blink_value = led->rgb_cfg->pwm_cfg->blinking;//ZTE ADD 
		pwm_free(pwm_cfg->pwm_dev);
		qpnp_pwm_init(pwm_cfg, led->spmi_dev, led->cdev.name);
		qpnp_led_set(&led->cdev, led->cdev.brightness);
	}
...
static DEVICE_ATTR(idx_len, 0664, NULL, idx_len_store);	/*zte-ccb-20130903 add*/
...
static struct attribute *lpg_attrs[] = {
	...
	&dev_attr_idx_len.attr,			/*zte-ccb-20130903 add*/
				...
				struct device_node *node)
{
	...
	if (pwm_cfg->mode == LPG_MODE || pwm_cfg->use_blink) {
		...
		#if 0
		pwm_cfg->lut_params.lut_pause_hi = 500;	/* zte-ccb-20130903
		...
		pwm_cfg->lut_params.lut_pause_lo = 500;	/* zte-ccb-20130903
		...
		if (!rc)
			...
			goto bad_lpg_params;	*/
		#else
		//change LED blink period to 7.5s,on=0.5s off=7s  for P897U11
		pwm_cfg->lut_params.lut_pause_hi = 250;
		pwm_cfg->lut_params.lut_pause_lo = 3500;
		#endif

		pwm_cfg->lut_params.ramp_step_ms =1;	/* zte-ccb-20130903
		...
		pwm_cfg->lut_params.flags = QPNP_ZTE_PWM_FLAGS;	/* zte-ccb-20130903
		...
		pwm_cfg->lut_params.idx_len = 3;	/* zte-ccb-20130903
			pwm_cfg->duty_cycles->num_duty_pcts;	*/
	}

	printk(LED_DRIVER_TAG "mode=%u; channel=%d; period_us=%u; use_blink=%d\n",
		pwm_cfg->mode,pwm_cfg->pwm_channel,pwm_cfg->pwm_period_us,pwm_cfg->use_blink);	//zte-ccb-20130903
	...
	for_each_child_of_node(node, temp) {
		...
		if (led->default_on) {
			...
			if(led->id == QPNP_ID_WLED)
				read_wled_default_brightness(led);
			__qpnp_led_work(led, led->cdev.brightness);
