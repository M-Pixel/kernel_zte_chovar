...
//jht 20131008 add for white/black list +++
#define htod32(i) i
#define htod16(i) i
#define dtoh32(i) i
#define dtoh16(i) i
...
int wl_add_maclist_filter(struct net_device *dev, char *command, int num, int mac_mode, int offset)
{
	int i = 0, ret = 0, length;
	int mac_list_num = (num - offset) / sizeof(struct mac_acl_entry);

	struct mac_acl_entry * mac_list;
	struct mflist *mac_filter_list;
	...
	{
		ret = wldev_ioctl(dev, WLC_SET_MACMODE, &mac_mode, sizeof(mac_mode), 1);
		printk("shaohua test %s, maclist_disable ret = %d \n", __func__, ret);
		return ret;
	}


	mac_list = (struct mac_acl_entry *)(command + offset);
	mac_filter_list= (struct mflist *)(kmalloc(sizeof(struct mflist), GFP_ATOMIC));

	printk("shaohua command length = %d, mac_list num = %d\n", num, mac_list_num);
	...
	for(i = 0; i < mac_list_num; i++)
		printk("hostapd send us mac=%02x:%02x:%02x:%02x:%02x:%02x \n",
				mac_list[i].addr[0],mac_list[i].addr[1],mac_list[i].addr[2],
				mac_list[i].addr[3],mac_list[i].addr[4],mac_list[i].addr[5]);

	ret = wldev_ioctl(dev, WLC_SET_MACMODE, &mac_mode, sizeof(mac_mode), 1);
	printk("shaohua test %s, set mac mode ret = %d \n", __func__, ret);

	//add maclist to white_maclist
	mac_filter_list->count = mac_list_num;
	for(i = 0; i < mac_list_num; i++) {
		memcpy(mac_filter_list->ea[i].octet, mac_list[i].addr, 6);
	}

	//set maclist to firmware
	length = sizeof(mac_filter_list->count) + mac_filter_list->count * sizeof(struct ether_addr);
	ret = wldev_ioctl(dev, WLC_SET_MACLIST, mac_filter_list, length, 1);

	/* deauth if there is associated station in black list */
	if (mac_mode == MACLIST_MODE_DENY)
	{
		...
		if (assoc_maclist->count) {
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				for (j = 0; j < mac_filter_list->count; j++) {
					if (!bcmp(&assoc_maclist->ea[i], &mac_filter_list->ea[j], ETHER_ADDR_LEN)) {
						DHD_ERROR(("match black, deauth it\n"));
						scbval.val = htod32(1);
...
//jht 20140109 add for max assoc +++
static int wl_android_set_maxassoc(struct net_device *dev, char *command, int total_len)
{
	int max_assoc = 0;

	max_assoc = command[20];
	printk("Enter %s, max_assoc = %d\n", __func__, max_assoc);
	if(max_assoc)
		if(wldev_iovar_setint(dev, "maxassoc", max_assoc))
			printk("fail set maxassoc, error = \n");
	return 0;
}
//jht 20140109 add for max assoc ---
...
//jht 20131008 add for white/black list +++
		...
		bytes_written = wl_add_maclist_filter(net, priv_cmd.buf, priv_cmd.total_len, MACLIST_MODE_ALLOW, 20);
	}
	else if(strnicmp(command, "add_black_list", strlen("add_black_list")) == 0) {
		printk("shaohua got add_black_list command \n");
		bytes_written = wl_add_maclist_filter(net, priv_cmd.buf, priv_cmd.total_len, MACLIST_MODE_DENY, 20);
	}
	else if(strnicmp(command, "disable_macfilter", strlen("disable_macfilter")) == 0) {
		printk("shaohua got disable macfilter command \n");
		bytes_written = wl_add_maclist_filter(net, priv_cmd.buf, priv_cmd.total_len, MACLIST_MODE_DISABLED, 20);
	}
#endif
//jht 20131008 add for white/black list ---
...
#endif /* WL_SUPPORT_AUTO_CHANNEL */
//jht 20140109 add for max assoc +++
	else if (strnicmp(command, "set_max_assoc",
		strlen("set_max_assoc")) == 0) {
		bytes_written = wl_android_set_maxassoc(net, command,
			priv_cmd.total_len);
	}
//jht 20140109 add for max assoc ---
