...
//u32 wl_dbg_level = 0xf; //WL_DBG_SCAN;
u32 wl_dbg_level = 0x1; //WL_DBG_ERR;
...
#endif /* PNO_SUPPORT */
//jht 20131217 add for max assoc notification ---
/*static void wl_cfg80211_hotspot_event_process(struct net_device *ndev, const wl_event_msg_t *e, void *data)
{
	...
	switch (event_type) {
		case WLC_E_PRUNE:
		{
			...
			if (1) { //(wl_android_black_list_match(macaddr)) {
				printf("STA block\n");
...
static s32
wl_set_maxassoc_status(struct wl_priv *wl, struct net_device *ndev,
		const wl_event_msg_t *e, void *data)
{
	u32 event_type = ntoh32(e->event_type);
	u32 status = ntoh32(e->status);
	u32 reason = ntoh32(e->reason);
	//NL_80211_CMD_STA_LIMIT
	printk("shaohua enter %s, type=%d, status=%d, reason=%d\n", __func__, event_type, status, reason);
	cfg80211_sta_limit(ndev, NULL, GFP_ATOMIC);
	return 0;
...
#endif /* WLTDLS */
//jht 20131217 add for max assoc notification +++
	wl->evt_handler[WLC_E_PRUNE] = wl_set_maxassoc_status;
//jht 20131217 add for max assoc notification ---
}
...
//jht 20140114 add for auto channel +++
