...
//zte VIB change to 2.9V for 8974 20140428S
#define QPNP_VIB_DEFAULT_VTG_LVL	2900
...
static int qpnp_vib_set(struct qpnp_vib *vib, int on)
{
	...
	u8 val;
	if (on) {
		//zte VIB 20140428 (sys/module/qpnp_vibrator/parameter/vib_voltage)
		if(vibrator_voltage > QPNP_VIB_MIN_LEVEL  && vibrator_voltage < QPNP_VIB_MAX_LEVEL){
			vib->vtg_level = vibrator_voltage;
			pr_info("vib_voltage reset to %d\n",vibrator_voltage);
		}
		printk(VIB_DRIVER_TAG "vib_on , voltage=%d\n",vib->vtg_level);
		...
		if (rc < 0)
			return rc;
		//printk(VIB_DRIVER_TAG "enable work reg_vtg_ctl(0x%x)\n",val);	//zte-ccb => 0x1F
		vib->reg_vtg_ctl = val;
		val = vib->reg_en_ctl;
		val |= QPNP_VIB_EN;
		...
		if (rc < 0)
			return rc;
		//printk(VIB_DRIVER_TAG "enable work reg_en_ctl(0x%x)\n",val);	//zte-ccb => 0x80
		vib->reg_en_ctl = val;
	} else {
		val = vib->reg_en_ctl;
		val &= ~QPNP_VIB_EN;
...
static enum hrtimer_restart qpnp_vib_timer_func(struct hrtimer *timer)
{
	struct qpnp_vib *vib = container_of(timer, struct qpnp_vib,
							 vib_timer);
	//printk(VIB_DRIVER_TAG "qpnp_vib_timer_func() timeout\n");	//zte-ccb
	vib->state = 0;
	schedule_work(&vib->work);
...
static int __devinit qpnp_vibrator_probe(struct spmi_device *spmi)
{
	...
	printk(VIB_DRIVER_TAG "probe(): vib->vtg_level(%d); vib->base(%x)\n",vib->vtg_level,vib->base);	//zte-ccb
	...
	printk(VIB_DRIVER_TAG "probe(): vib->reg_vtg_ctl(%x); vib->reg_en_ctl (%x)\n",vib->reg_vtg_ctl,vib->reg_en_ctl );	//zte-ccb
	...
	vib_dev = vib;
	//qpnp_vib_enable(&vib->timed_dev,200);	//zte-ccb vibrate when probe. 100ms  //put it into aboot
