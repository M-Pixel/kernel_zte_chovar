...
{
	int32_t index;
	const char *ptr = NULL, *name = NULL;

	if (dev->kobj.name) {
		ptr = dev->kobj.name + strlen(msm_sensor_sysdev_class.name);
		index = (int32_t)simple_strtol(ptr, NULL, 10);
		name = msm_sensor_sysdev_info[index].name;
	} else {
		name = "invalid";
	}
...
/*
 * Modified for camera sensor by ZTE_JIA_20130702 jia.jia
 * Configure GPIO in power-down mode
 */
#if 1
	if (of_property_read_bool(of_node, "qcom,gpio-ldo") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-ldo", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-ldo failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ldo invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_LDO] =
			gpio_array[val];

		CDBG("%s qcom,gpio-ldo %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_LDO]);
	}
#endif
...
/*
 * Support for performance improvement of camera, DISUSED here
 * Modified by ZTE_JIA_20131012 jia.jia
 */
//#if defined(CONFIG_MSM_CAMERA_PERF)
#if 0
...
/*
 * Added by ZTE_JIA_20130620 jia.jia
 */
#if defined(CONFIG_MSM_CAMERA_EMODE)
