...
/*
 * Support for camera actuator by ZTE_BOOT_JIA_20130926, jia.jia
 */
#if 0
...
/*
 * Support for camera actuator of Rohm bu64291 by ZTE_BOOT_JIA_20130516, jia.jia
 */
#if defined(CONFIG_BU64291)
/*
...
/*
 * Fix potential problem of CCI init/release
 ...
 */
		...
		if (rc < 0) {
			pr_err("cci_init failed\n");
			(void)a_ctrl->i2c_client.i2c_func_tbl->i2c_util(
...
#else
int32_t msm_actuator_platform_probe(struct platform_device *pdev)
#endif
{
	int32_t rc = 0;
	struct msm_camera_cci_client *cci_client = NULL;
 ...
 */
		...
		return rc;
#else
		goto msm_actuator_platform_probe_failed;
#endif
	}
 ...
 */
#if 1
	rc = of_property_read_u32((&pdev->dev)->of_node, "qcom,slave-addr", &slave_addr);
	CDBG("qcom,slave-addr %d, rc %d\n", slave_addr, rc);
	if (rc < 0) {
		pr_err("failed rc %d\n", rc);
		goto msm_actuator_platform_probe_failed;
	}
	...
	rc = of_property_read_string((&pdev->dev)->of_node, "qcom,actuator-name", &msm_actuator_t->actuator_name);
	if (rc < 0) {
		pr_err("failed rc %d\n", rc);
		goto msm_actuator_platform_probe_failed;
	}

	pr_info("%s %s probe succeeded\n", __func__, msm_actuator_t->actuator_name);
#endif
...
msm_actuator_platform_probe_failed:

	if (msm_actuator_t->i2c_client.cci_client) {
		kfree(msm_actuator_t->i2c_client.cci_client);
		msm_actuator_t->i2c_client.cci_client = NULL;
	}
