...
/*
 * Modified for camera eeprom by ZTE_JIA_20130704 jia.jia
 * Fix bug of incorrect slave address and register address
 * Refer to kernel/arch/arm/boot/dts/msm8974-camera-eeprom-mtp.dtsi
 */
#if 0
...
/*
 * Fix memory leak by ZTE_JIA_20131113 jia.jia
 */
#if 0
		return -EINVAL;
#else
		rc = -EINVAL;
		goto msm_eeprom_platform_probe_failed;
#endif
	}
...
/*
 * Commented to fix core dump if 'MSM_EEPROM_DEBUG' is enabled
 * by ZTE_JIA_20131113 jia.jia
 */
#if 0
	CDBG("%s line %d\n", __func__, __LINE__);
	for (j = 0; j < e_ctrl->num_bytes; j++)
...
/*
 * Added for camera eeprom by ZTE_JIA_20130627 jia.jia
 * To power up/down camera eeprom/sensor
 */
#if 1
	pr_info("%s %s probe succeeded\n", __func__, e_ctrl->eboard_info->eeprom_name);

	CDBG("%s num_blocks %d\n", __func__, e_ctrl->eboard_info->num_blocks);

	CDBG("%s block 0 info:\n", __func__);
	for (j = 0; j < e_ctrl->eboard_info->eeprom_map[0].mem.valid_size; ++j) {
		CDBG("offset: 0x%x, value: 0x%x\n", j, e_ctrl->memory_data[j]);
	}
#endif
...
cciclient_free:
	...
	return rc;
#else
msm_eeprom_platform_probe_failed:

	if (e_ctrl->memory_data) {
		kfree(e_ctrl->memory_data);
		e_ctrl->memory_data = NULL;
	}

	if (eb_info->eeprom_map) {
		kfree(eb_info->eeprom_map);
		eb_info->eeprom_map = NULL;
	}

	if (e_ctrl->eboard_info) {
		kfree(e_ctrl->eboard_info);
		e_ctrl->eboard_info = NULL;
	}

	if (e_ctrl->i2c_client.cci_client) {
		kfree(e_ctrl->i2c_client.cci_client);
		e_ctrl->i2c_client.cci_client = NULL;
	}
	...
	pr_err("%s msm_camera_power_down rc %d\n", __func__, rc);
