...
//ZTE
#define ZTE_VBAT_TOLERANCE_MV  70
...
struct qpnp_chg_chip {
	...
	struct delayed_work		update_heartbeat_work;  //ZTE
	struct delayed_work             bms_check_work;  //ZTE
	...
	bool				power_stage_workaround_running;
        struct wake_lock charger_wake_lock;   //ZTE XJB add
        struct wake_lock charger_fulled_lock;  //ZTE
...
static int count_number =15;    //ZTE_XJB
static bool flag_check=false;     //ZTE
						...
						u8 mask, u8 val, int count)
{
	int rc;
	u8 reg;
//jht 20140113 add to disable ULPM +++
	if (base == 0x12DF) {
		dump_stack();
		printk("%s, val=%02x, mask=%02x\n", __func__, val, mask);
	}
//jht 20140113 add to disable ULPM ---
...
qpnp_chg_is_dc_chg_plugged_in(struct qpnp_chg_chip *chip)
{
	...
	if (rc) {
        ...
        pr_debug("ZTE dcin_valid_rt_sts=%d \n",dcin_valid_rt_sts);
...
printk("ZTE chip->maxinput_usb_ma=%d,mA=%d \n",chip->maxinput_usb_ma,mA);
...
{
        pr_debug("use_default_batt_values=%d,disable=%d \n",chip->use_default_batt_values,disable);
...
{
        pr_debug("chip->bat_is_cool=%d,bat_is_warm=%d,resuming_charging=%d\n",chip->bat_is_cool,chip->bat_is_warm,chip->resuming_charging);
	if (chip->bat_is_cool)
...
qpnp_arb_stop_work(struct work_struct *work)
{
	...
	struct qpnp_chg_chip *chip = container_of(dwork,
        ...
        printk("%s,USB_present=%d \n",__func__,qpnp_chg_is_usb_chg_plugged_in(chip)); //XJB
        printk("%s,chip->chg_done=%d \n",__func__,chip->chg_done); //XJB
	if (!chip->chg_done)
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	struct qpnp_chg_chip *chip = _chip;
        u8 usb_sts;
        int rc;
	int usb_valid,dc_valid;
	bool usb_present_check;
	...
	count_number=15;   //ZTE
	usb_valid = qpnp_chg_is_usb_chg_plugged_in(chip);
       dc_valid  = qpnp_chg_is_dc_chg_plugged_in(chip);
	printk("%s,usb_valid=%d,dc_valid=%d--------!\n",__func__,usb_valid,dc_valid);

	if(usb_valid||dc_valid)
		usb_present_check	=	true;
	else
		usb_present_check =	false;
	printk("%s,charger insert/remove:old=%d,new=%d,(usb_sts & CHG_GONE_IRQ)=%ld\n",__func__,usb_valid_last,usb_present_check,(usb_sts & CHG_GONE_IRQ));
	usb_valid_last = usb_present_check;
#if 0
	if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
#else
	if (usb_present_check&& (usb_sts & CHG_GONE_IRQ)) {
#endif
	...
	{
	        printk("CHG GONE. ZTE update power change\n");
		power_supply_changed(&chip->batt_psy);
		//power_supply_changed(chip->usb_psy);
	}
	return IRQ_HANDLED;
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	host_mode = qpnp_chg_is_otg_en_set(chip);
	printk("usbin-valid triggered: %d host_mode: %d,chip->usb_present =%d\n",
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			qpnp_chg_usb_suspend_enable(chip, 0);
 		        //chip->chg_done = false;  //ZTE add.
			wake_unlock(&chip->charger_fulled_lock);   //ZTE
...
qpnp_chg_dc_dcin_valid_irq_handler(int irq, void *_chip)
{
	...
	printk("dc insert/remove isr:old=%d,new=%d--------!\n",chip->dc_present,dc_present);
...
qpnp_chg_chgr_chg_fastchg_irq_handler(int irq, void *_chip)
{
	...
	printk("%s FAST_CHG IRQ triggered\n", __func__);
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		battery_v=get_prop_battery_voltage_now(chip);
		printk("%s,battery_v=%d,soc=%d ,old g_bms_flag=%d\n",__func__,battery_v,soc,g_bms_flag);
		if ((first_time_run>=1)&&(battery_v>3200000))
		{
			printk("ZTE get_prop_batt_capacity voltage = %d,first_time_run=%d,soc=%d%%\n", battery_v,first_time_run,soc);
			first_time_run--;
			if (soc <= 3)
			{
				if(soc==0)
		...
		if (((soc <3)&&(battery_v <3400000))||((soc <5)&&(battery_v <3200000)))
		{
			printk("ZTE low battery capacity = %d%%,voltage=%d,double_check=%d\n", soc,battery_v,checkflag);
			//#if defined(CONFIG_MACH_HAYES)||defined(CONFIG_MACH_ELDEN) //ZTE_XJB_PM_20120831 start
			if(checkflag>=10)
			{
				//double_check=0;
				soc = 0;
				printk("%s line %d,batt percent_soc seting 0 here as voltage and no charging\n",__func__,__LINE__);
			}
			else{
				checkflag++;
		...
		//ZTE
		if (soc <= 5)
			printk("ZTE  soc=%d,g_bms_flag=%d,flag_check=%d ,count_number=%d\n",soc,g_bms_flag,flag_check,count_number);
		...
		if (soc == 0) {
			...
			//zte add
			if((!enable_to_shutdown)||(battery_v>3500000))
			{
			        g_bms_flag=100;
				return 1;
			}
		...
		if(!flag_check)
			schedule_delayed_work(&chip->bms_check_work,round_jiffies_relative(msecs_to_jiffies(500)));
		...
		if(soc==0)
			{
			printk( "get_prop_batt_capacity: HAHA shutdown as percent_soc=%d ,report_zero =%d\n",  soc,report_zero);
			report_zero=true;
			}
		else
			report_zero=false;
...
/*   To check current in/out   */
static void bms_check(struct work_struct *work)
{
	int capacity,chg_current,status;

	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,struct qpnp_chg_chip, bms_check_work);

        if(chip==NULL)
      	{
          printk("pm8941 fatal error:the_chip=null\n!!");
      	 }
	 else if(chip->bms_psy)
		{
		//temperature =get_prop_batt_temp(chip)/10; 
		//voltage=get_prop_battery_uvolts(chip)/1000;
		capacity	=	get_prop_capacity(chip);
		//present= is_battery_valid(chip);
		chg_current	=	get_prop_current_now(chip)/1000;
		...
		if((count_number>=30)||(count_number<=-10))
		count_number=15;
		//printk( "bms_check: battery temp = %d,voltage=%d,capacity=%d,current=%d,present=%d,count_number=%d\n",  temperature,voltage,capacity,chg_current,present,count_number);
		printk( "bms_check: battery capacity=%d,current=%d\n",  capacity,chg_current);

		//if ((capacity<=3)&&(status==POWER_SUPPLY_STATUS_CHARGING))
                if (capacity<=3)
		{
		schedule_delayed_work(&chip->bms_check_work,round_jiffies_relative(msecs_to_jiffies(1000)));
		flag_check=true;
		}
		else
		{
...
{
printk("ZTE lvl_sel=%d,chip->thermal_levels=%d \n",lvl_sel,chip->thermal_levels);  //ZTE
...
qpnp_chg_adjust_vddmax(struct qpnp_chg_chip *chip, int vbat_mv)
{
	...
	if (delta_mv > 0 && delta_mv < MIN_DELTA_MV_TO_INCREASE_VDD_MAX) {
		...
		pr_debug("max_voltage = %d, vbat_mv = %d, delta_mv = %d\n",chip->max_voltage_mv, vbat_mv,delta_mv);
		return;
	}
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		if (!(buck_sts & VDD_LOOP_IRQ)) {
			...
			count = 0;
            	...
            	//zte jiangfeng add
		else if(vbat_mv<(chip->max_voltage_mv-ZTE_VBAT_TOLERANCE_MV))
		{
			count = 0;
		pr_debug("ZTE  WARNING:vbat_mv<(chip->max_voltage_mv-ZTE_VBAT_TOLERANCE_MV)\n");
		}
...
qpnp_chg_request_irqs(struct qpnp_chg_chip *chip)
{
	...
	spmi_for_each_container_dev(spmi_resource, chip->spmi) {
		...
		case SMBCL_BAT_IF_SUBTYPE:
			...
			//enable_irq_wake(chip->batt_pres.irq);		//zte jiangfeng remove
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_CHGR_SUBTYPE:
		//XJB for VDDMAX+++
		rc=qpnp_chg_masked_write(chip, 0x1043, 0x80, 0, 1);
		if (rc) {
			pr_debug("failed setting 0x1043=%d\n", rc);
		}
		//XJB for VDDMAX---
	...
	case SMBCL_BAT_IF_SUBTYPE:
		...
		if (rc) {
			pr_debug("failed to disable  ULPM rc=%d\n", rc);
		}
		//jht 20140113 add to disable ULPM ---
...
qpnp_charger_read_dt_props(struct qpnp_chg_chip *chip)
{
	...
	printk("ZTE before hot_batt_p=%d ,cold_batt_p=%d\n",chip->hot_batt_p,chip->cold_batt_p);
#if 1  //ZTE  set 0-60 degree
chip->hot_batt_p=HOT_THD_25_PCT;
chip->cold_batt_p=COLD_THD_80_PCT;
#endif
	printk("ZTE after hot_batt_p=%d ,cold_batt_p=%d\n",chip->hot_batt_p,chip->cold_batt_p);

	printk("XJB term_current=%d ,maxinput_dc_ma=%d,maxinput_usb_ma=%d,tchg_mins=%d\n",chip->term_current,chip->maxinput_dc_ma,chip->maxinput_usb_ma,chip->tchg_mins);
	printk("XJB min_voltage_mv=%d ,max_voltage_mv=%d,safe_current=%d,max_bat_chg_current=%d\n",chip->min_voltage_mv,chip->max_voltage_mv,chip->safe_current,chip->max_bat_chg_current);
	...
	printk("ZTE chip->use_default_batt_values=%d,chip->charging_disabled=%d",chip->use_default_batt_values,chip->charging_disabled);
	chip->charging_disabled = false;		//zte enable charging default
...
static void update_heartbeat(struct work_struct *work)
{
	...
	if(chip==NULL)
	{
		printk("pmic fatal error:the_chip=null\n!!");
		return;
	}
	...
	printk("update_heartbeat: battery temp = %d,voltage=%d,capacity=%d,batt_status=%d,chg_state=%d,current=%d,batt_present=%d,usb_present=%d@%d@%d\n",  
		                               temperature,voltage,capacity,status,charge_type,chg_current,present,chip->usb_present,zte_chip->usb_present,usb_valid_last);
	printk("update_heartbeat: charging enabled = %d,chip->bat_is_cool=%d,chip->bat_is_warm=%d\n",qpnp_chg_charge_en_state(chip),chip->bat_is_cool,chip->bat_is_warm);

	power_supply_changed(&chip->batt_psy);
	//ZTE add for kernel SOC 0%  shutdown when services not shutdown successful.
	if(report_zero)
	{
		if(offcharging_flag)
		{
			if(((num_c++)>30)&&(capacity==0))
			{
				printk("[FTM] kernel shutdown for %d *30 seconds \n",num_c);  //10 minutes pretect
				kernel_power_off();   //ZTE
			}
		}
		else if((num_c++)>2)
		{
			printk("kernel shutdown for %d *20 seconds \n",num_c);  //60s pretect
			kernel_power_off();   //ZTE
		}
	}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
...
#if 0   //ZTE remove old battery data
	...
	spmi_for_each_container_dev(spmi_resource, spmi) {
		...
		case SMBCL_CHGR_SUBTYPE:
			chip->chgr_base = resource->start;
			 printk("ZTE SMBB_CHGR_SUBTYPE chg->chgr_base=%d \n",chip->chgr_base);  //zte
		...
		case SMBCL_BUCK_SUBTYPE:
			...
			printk("ZTE SMBB_BUCK_SUBTYPE chg->buck_base=%d \n",chip->buck_base);  //zte
		...
		case SMBCL_BAT_IF_SUBTYPE:
			...
			printk("ZTE SMBB_BAT_IF_SUBTYPE chg->bat_if_base=%d \n",chip->bat_if_base);  //zte
		...
		case SMBCL_USB_CHGPTH_SUBTYPE:
			...
			printk("ZTE SMBB_USB_CHGPTH_SUBTYPE chg->usb_chgpth_base=%d \n",chip->usb_chgpth_base);  //zte
		...
		case SMBB_DC_CHGPTH_SUBTYPE:
			...
			rc = qpnp_chg_hwinit(chip, subtype, spmi_resource);
			printk("ZTE SMBB_DC_CHGPTH_SUBTYPE chg->dc_chgpth_base=%d \n",chip->dc_chgpth_base);  //zte
		...
		case SMBBP_BOOST_SUBTYPE:
			...
			printk("ZTE SMBB_BOOST_SUBTYPE chg->boost_base=%d \n",chip->boost_base);  //zte
		...
		case SMBCL_MISC_SUBTYPE:
			...
			printk("ZTE SMBB_MISC_SUBTYPE chg->misc_base=%d \n",chip->misc_base);  //zte
	...
	INIT_DELAYED_WORK(&chip->bms_check_work, bms_check);   //ZTE add
