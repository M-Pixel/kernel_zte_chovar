...
static void reset_for_new_battery(struct qpnp_bms_chip *chip, int batt_temp)
{
	...
	chip->shutdown_soc = 0;
	chip->shutdown_iavg_ma = MIN_IAVG_MA;  //SBA
			...
			int batt_temp)
{
	...
	pr_debug("new_pc_unusable =%d new_uuc_uah = %d batt_temp = %d,chip->prev_pc_unusable=%d,max_percent_change=%d,chip->first_time_calc_uuc =%d\n",
					new_pc_unusable, new_uuc_uah,batt_temp,chip->prev_pc_unusable,max_percent_change,chip->first_time_calc_uuc);    //ZTE
					...
					int batt_temp)
{
	...
	pr_debug("%s chip->first_time_calc_uuc=%d,chip->shutdown_iavg_ma=%d \n",__func__,chip->first_time_calc_uuc,chip->shutdown_iavg_ma);
	if (chip->first_time_calc_uuc && chip->shutdown_iavg_ma != 0) {
		pr_debug("Using shutdown_iavg_ma = %d in all samples\n",
				chip->shutdown_iavg_ma);
		pr_info("Using shutdown_iavg_ma = %d in all samples\n",
				chip->shutdown_iavg_ma);
				...
				int vbat_uv, int ibat_ua, int batt_temp)
{
	...
	printk("ZTE vbat_uv= %d,ibat_ua=%d,params->iavg_ua=%d\n", vbat_uv,ibat_ua,params->iavg_ua);//ZTE
...
out:
	pr_debug("ibat_ua = %d, vbat_uv = %d, ocv_est_uv = %d, pc_est = %d, soc_est = %d, n = %d, delta_ocv_uv = %d, last_ocv_uv = %d, pc_new = %d, soc_new = %d, rbatt = %d, slope = %d\n",
        ...
        battery_voltage=ocv_est_uv/1000;     //ZTE
	pr_debug("ZTE battery_voltage = %d \n ",battery_voltage);
	return soc;
}
...
static int clamp_soc_based_on_voltage(struct qpnp_bms_chip *chip, int soc)
{
	...
	if (soc == 0 && vbat_uv > chip->v_cutoff_uv) {
		...
		pr_info("adjust soc from 0 to 1, vbat (%d) > cutoff (%d)\n",
						vbat_uv, chip->v_cutoff_uv);
		return 1;
	} else {
					...
					int batt_temp)
{
	...
	soc = DIV_ROUND_CLOSEST((remaining_usable_charge_uah * 100),
        ...
        pr_info("RUC=%d,FCC=%d,UUC=%d\n", remaining_usable_charge_uah,params->fcc_uah,params->uuc_uah);
	if (chip->first_time_calc_soc && soc > BAD_SOC_THRESH && soc < 0) {
		 ...
		 */
		pr_info("first time, soc %d, batt temp %d, rbatt %d, \
				fcc_uah %d, unusable_charge_uah %d, cc_uah %d\n",
				soc, batt_temp, params->rbatt_mohm, 
				params->fcc_uah, params->uuc_uah, params->cc_uah);
...
//zte jiangfeng add
	if(soc == 100	&& is_battery_charging(chip))
	{
		printk("%s ZTE  for charging restore soc to 99,here. \n",__func__);
		soc	=	99;
	}
					...
					int batt_temp)
{
	...
	shutdown_soc = chip->shutdown_soc;
		printk("calculate_state_of_charge shutdown_soc=%d \n",shutdown_soc);
	...
	new_calculated_soc = clamp_soc_based_on_voltage(chip,
					new_calculated_soc);
	pr_debug("soc %d, batt temp %d, rbatt %d, \
		fcc_uah %d, uuc_uah %d, cc_uah %d\n",
		soc, batt_temp, params.rbatt_mohm, 
		params.fcc_uah, params.uuc_uah, params.cc_uah);
	/*
...
static int recalculate_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->use_voltage_soc) {
		soc = calculate_soc_from_voltage(chip);
		pr_info("calculate soc from voltage,soc=%d\n", soc);
	} else {
		if (!chip->batfet_closed)
			qpnp_iadc_calibrate_for_trim(chip->iadc_dev, false);
		rc = qpnp_vadc_read(chip->vadc_dev, LR_MUX1_BATT_THERM,
		...
		if (rc) {
			...
			mutex_lock(&chip->last_ocv_uv_mutex);
			read_soc_params_raw(chip, &raw, batt_temp);
			pr_info("ocv_raw= 0x%x, ocv_uv= %duV,cc = %lld\n",
					raw.last_good_ocv_raw, raw.last_good_ocv_uv, raw.cc);		//zte jiangfeng
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	printk("pm8921 read battery_id NO USED =0x%llX,FCC=%d\n",battery_id,palladium_data->fcc);

	chip->fcc_mah = palladium_data->fcc;
	...
	chip->rbatt_capacitive_mohm = palladium_data->rbatt_capacitive_mohm;
	chip->flat_ocv_threshold_uv = palladium_data->flat_ocv_threshold_uv;

	printk("ZTE again set_battery_data  chip->max_voltage_uv=%d,chip->v_cutoff_uv=%d,chip->chg_term_ua=%d\n",chip->max_voltage_uv,\
		chip->v_cutoff_uv,chip->chg_term_ua);
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	batt_data = &palladium_1500_data;   //ZTE
	printk("ZTE set_battery_data 3200_data\n");//ZTE
...
static int register_spmi(struct qpnp_bms_chip *chip, struct spmi_device *spmi)
{
	...
	chip->spmi = spmi;
        ...
        printk("ZTE register_spmi running\n");//ZTE
	...
	spmi_for_each_container_dev(spmi_resource, spmi) {
		...
		printk("ZTE register_spmi type=%d,subtype=%d \n \n",type,subtype);//ZTE
...
static int __devinit qpnp_bms_probe(struct spmi_device *spmi)
{
	...
	printk("bms probe, soc =%d vbatt = %d ocv = %d r_sense_uohm = %u warm_reset = %d\n",
			get_prop_bms_capacity(chip), vbatt, chip->last_ocv_uv,
			chip->r_sense_uohm, warm_reset);
	return 0;
