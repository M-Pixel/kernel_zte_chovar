...
static int32_t auth_sec_cert_codeseg_hash(const void __user *umod, const auth_sec_file_t *as)
{
	 ...
	 */
	data_offset = as->phdr[AUTH_SEC_PHNUM - 2].p_offset + MI_BOOT_IMG_HDR_SIZE + (SHA1_DIGEST_SIZE * 2);

	data_len = SHA1_DIGEST_SIZE;
	 ...
	 */
	if (0 != memcmp((const void *)data, (const void *)hash, SHA1_DIGEST_SIZE)) {
		ret = -EINVAL;
		goto auth_sec_cert_code_seg_done;
	}
