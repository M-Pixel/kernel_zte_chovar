...
//jht 20130910 add for mac addr +++
#include <linux/fs.h>
//jht 20130910 add for mac addr ---
#define BCM_GPIO_WIFI_SHUTDOWN_N	76
#define BCM_GPIO_WIFI_IRQ		75

//#define GET_WIFI_MAC_ADDR_FROM_NV_ITEM	1
//jht 20130910 add for mac addr +++
#define GET_WIFI_MAC_ADDR_FROM_FILE
//jht 20130910 add for mac addr ---
...
int bcm_wifi_power(int on)
{
	...
	if(on) {
		if((gpio_request(BCM_GPIO_WIFI_SHUTDOWN_N,"shaohua")) < 0) {
			printk("request fail \n");
		} else {
	...
	printk("shaohua gpio_get_value = %d \n",
...
#endif
//jht 20130910 add for mac addr +++
#ifdef GET_WIFI_MAC_ADDR_FROM_FILE
...
int zte_wifi_get_mac_addr(unsigned char *addr)
{
	...
	if (!s) {
		...
		if (0 != ((unsigned int)addr[0] % 2)) {
			...
			printk("Got wifi mac successfully! \n");
