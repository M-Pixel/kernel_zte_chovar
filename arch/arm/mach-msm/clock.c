...
#if 1
	if (!strcmp(clk->dbg_name, "bimc_msmbus_a_clk"))
	{
                 printk("bimc_msmbus_a_clk is enable,prepare_count=%d,count=%d\n",clk->prepare_count,clk->count );
	}
#endif
	#ifdef ZTE_CLK_DEBUG
	if(clk_debug_zte)
...
//		printk("zte_clk %test\n",clk_debug_zte);
		if (strstr(clk->dbg_name,clk_debug_zte))
			WARN(1,"ZTE_PM_CLK  %s:  enable  clocks\n", clk->dbg_name);
	}
	#endif
...
void clk_disable(struct clk *clk)
{
	...
	if (!strcmp(clk->dbg_name, "bimc_msmbus_a_clk"))
	{
                 printk("bimc_msmbus_a_clk is disable,prepare_count=%d,count=%d\n",clk->prepare_count,clk->count );
	}
#endif
	#ifdef ZTE_CLK_DEBUG
	if(clk_debug_zte)
	{
		if (strstr(clk->dbg_name,clk_debug_zte))
			WARN(1,"ZTE_PM_CLK %s:  disable  clocks\n", clk->dbg_name);
	}
	#endif
